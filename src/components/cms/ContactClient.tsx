      ...
      <section className="pt-28 pb-16 px-6 md:px-10">
        <div className="max-w-[1000px] mx-auto text-center md:text-left grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <p className="text-muted text-lg mb-10 leading-relaxed whitespace-pre-line">
                   ...
                   return section.items?.map((item, i) => (
                      <div key={`${index}-${i}`} className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-accent-soft flex items-center justify-center text-accent shrink-0">
                          {item?.label === "EMAIL" ? <Mail size={20} /> : <MapPin size={20} />}
                        </div>
                        <div className="text-left">
                          <span className="block text-sm font-bold text-foreground mb-1 uppercase tracking-widest">{item?.title}</span>
                          {item?.label === "EMAIL" ? (
                            <a href={`mailto:${item?.text}`} className="text-accent underline underline-offset-4 hover:opacity-80">
                              {item?.text}
                            </a>
                          ) : (
