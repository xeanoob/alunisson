...
export function RejoindreClient(props: {
  data: PageQuery;
      ...
      <section className="pt-28 pb-20 md:pb-32 px-6 md:px-10">
        <div className="max-w-[1400px] mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-12 lg:gap-16 items-end mb-14">
            ...
            >
              <Image 
                src={content.hero?.image || "/machine_zoom.png"} 
                alt="Machine à coudre Alunisson" 
                fill 
              ...
              {content.sections?.map((section, index) => {
                if (section?.__typename === "PageSectionsGridSection") {
                  return section.items?.map((item, i) => (
                    <div key={`${index}-${i}`} className={`${i === 0 ? "bg-accent-soft" : "bg-white border border-border"} rounded-2xl p-6`}>
                      <div className="flex items-center gap-2 mb-3">
                        {item?.label === "LIEU" && <MapPin size={14} className="text-accent" />}
                        {item?.label === "INFOS" && <Clock size={14} className="text-accent" />}
                        {item?.label === "EMAIL" && <Mail size={14} className="text-accent" />}
                        <span className="text-xs text-muted uppercase tracking-wider font-semibold">{item?.title}</span>
                      </div>
                      <p className={`text-sm leading-relaxed ${item?.label === "INFOS" ? "text-muted" : "text-foreground"}`}>
                        {item?.text}
                      </p>
                    </div>
                  ));
